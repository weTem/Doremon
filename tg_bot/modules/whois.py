...
def whois(bot: Bot, update: Update, args: List[str]):
    ...
    text = (f"<b>𝗨𝗦𝗘𝗥 𝗜𝗡𝗙𝗢𝗥𝗠𝗔𝗧𝗜𝗢𝗡</b>\n"
            f"ɪᴅ : <code>{user.id}</code>\n"
            f" ⍟ ɴᴀᴍᴇ: {html.escape(user.first_name)}")
    ...
    text += f"\n⍟ ɪɴʟɪɴᴇ ᴍᴇɴᴛɪᴏɴ: {mention_html(user.id, 'link🌩')}"

    num_chats = sql.get_user_num_chats(user.id)
    text += f"\n⍟ ᴄʜᴀᴛ ᴄᴏᴜɴᴛ: <code>{num_chats}</code>"
    text += "\n⍟ ɴᴏ ᴏғ ᴘʀᴏғɪʟᴇ ᴘɪᴄs: {}".format(bot.get_user_profile_photos(user.id).total_count)
    ...
    if user.id == OWNER_ID:
        text += "\n🚶🏻‍♂️Uff,💠 ᴛʜɪs ᴘᴇʀsᴏɴ ɪs ᴍʏ ᴏᴡɴᴇʀ , ɪ ᴄᴀɴ'ᴛ ᴅᴏ ᴀɴʏᴛʜɪɴɢ ᴀɢᴀɪɴsᴛ ʜɪᴍ! 💠!."
        
    elif user.id in DEV_USERS:
        text += "\n🚴‍♂️Pling,💠 ᴛʜɪs ᴘᴇʀsᴏɴ ɪs ᴍʏ ᴅᴇᴠᴇʟᴏᴘᴇʀ , ɪ ᴄᴀɴ'ᴛ ᴅᴏ ᴀɴʏᴛʜɪɴɢ ᴀɢᴀɪɴsᴛ ʜɪᴍ! 💠 ."
